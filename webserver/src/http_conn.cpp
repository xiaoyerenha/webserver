...
//网站的根目录
const char* doc_root = "/home/ma/code/code_c/webserver/resources";
...
bool http_conn::write() {
    ...
    if(bytes_to_send == 0) {//将要发送的字节为0，本次响应结束
...
http_conn::HTTP_CODE http_conn::parse_request_line(char * text) {
    ...
    *m_version++ = '\0';
    ...
    m_check_state = CHECK_STATE_HEADER; //主状态机
    return NO_REQUEST;
}
http_conn::HTTP_CODE http_conn::parse_request_headers(char * text) {
    ...
    //遇到空行，表示解析完毕
    if(text[0] == '\0') {
        if(m_content_length != 0) {//由消息体，状态机转移到CHECK_STATE_CONTENT状态
